...
class UIControls extends Component {
    ...
    const {showDialog} = this.props
		return (
			<React.Fragment>
				<div className="nav">
					{this.props.recordingStatus.isRecording && <div className="recording-bar"></div>}
					...
					<div className="flex">
						<div className="nav-left mt-2 ml-2 shadow flex">
							<button
								className={`btn btn-default rounded-l ${this.props.mode === MODE.LIVEVIEW ? "btn-default--active" : ""} ${
									!this.props.uiSettings.get("pathfinderEnabled") && !this.props.uiSettings.get("counterEnabled") ? "rounded-r" : ""
								}`}
								onClick={() => this.props.dispatch(setMode(MODE.LIVEVIEW))}
							>
								Стрим
							</button>
							{this.props.uiSettings.get("counterEnabled") && (!this.props.recordingStatus.isRecording || this.props.isAtLeastOneCountingAreasDefined) && (
								<button
									className={`btn btn-default border-r border-l border-default-soft border-solid ${this.props.mode === MODE.COUNTERVIEW ? "btn-default--active" : ""} ${
										this.props.uiSettings.get("pathfinderEnabled") ? "" : "rounded-r"
									}`}
							...
							{this.props.uiSettings.get("pathfinderEnabled") && (
								<button className={`btn btn-default ${this.props.mode === MODE.PATHVIEW ? "btn-default--active" : ""}`} onClick={() => this.props.dispatch(setMode(MODE.PATHVIEW))}>
									Трекер путей
								</button>
							)}
						...
						<div className="nav-right mt-2 mr-2 flex">
							...
							<button className={`btn btn-default shadow ml-2 py-0 px-3 rounded border border-default-soft border-solid`} onClick={() => this.props.dispatch(showMenu())}>
								<SVG className="w-5 h-5 svg-icon flex items-center" cacheRequests={true} src={`/static/icons/ui/menu.svg`} aria-label="icon menu" />
							</button>
						</div>
					</div>
				</div>
