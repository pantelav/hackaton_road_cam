...
import DialogFiles from "./main/DialogFiles";
		...
		return (
			<div className="main-page" onDragOver={(event) => event.preventDefault()} onDragStart={(event) => event.preventDefault()}>
				...
				{this.props.isListeningToYOLO && !this.state.droppedFile && (
					<>
						<UIControls showDialog={this.showDialog} />
						{this.props.showMenu && <Menu />}
						{this.props.mode === MODE.DATAVIEW && <DataView />}
						{this.props.mode === MODE.CONSOLEVIEW && <ConsoleView />}
						{this.props.mode === MODE.LIVEVIEW && <LiveView />}
						{this.props.uiSettings.get("counterEnabled") && this.props.mode === MODE.COUNTERVIEW && <CounterView />}
						{/* Need to keep pathview in the DOM as it continuously renders */}
						{this.props.uiSettings.get("pathfinderEnabled") && <PathView hidden={this.props.mode !== MODE.PATHVIEW} />}
