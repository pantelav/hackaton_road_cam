    ...
    return (
			<div className="overlay" onClick={() => this.props.close()}>
				<div className="modal rounded p-10 shadow text-inverse bg-default border-inverse">
					<h3 className="text-center text-xl font-bold">Точность трекера</h3>
					<div className="mt-4">
						<p>
							Тепловые обозначени показывают зоны где точность трекера <strong>низка</strong>, чтобы помочь Вам:
						</p>
					...
					<button className="btn btn-primary btn-rounded min-w-100 mt-5 pl-10 pr-10" onClick={() => this.props.close()}>
						OK
					</button>
				</div>
				<style jsx>{`
