...
class DBManager {
	...
	getAppSettings() {
		return new Promise((resolve, reject) => {
			this.getDB().then((db) => {
				db.collection(APP_COLLECTION).findOne({ id: "settings" }, (err, doc) => {
					if (err) {
						reject(err);
					} else {
						resolve(doc);
	...
	updateRecordingWithNewframe(recordingId, frameDate, counterSummary, trackerSummary, counterEntry, trackerEntry) {
		return new Promise((resolve, reject) => {
			// let itemsToAdd = {
			...
			this.getDB().then((db) => {
				db.collection(RECORDING_COLLECTION).updateOne({ _id: recordingId }, updateRequest, (err, r) => {
					if (err) {
						reject(err);
					} else {
						resolve(r);
	...
	getRecording(recordingId) {
		return new Promise((resolve, reject) => {
			this.getDB().then((db) => {
				db.collection(RECORDING_COLLECTION).findOne({ _id: ObjectID(recordingId) }, { projection: { counterHistory: 0, areas: 0 } }, (err, doc) => {
					if (err) {
						reject(err);
					} else {
						resolve(doc);
	...
	getRecordingsCount() {
		return new Promise((resolve, reject) => {
			this.getDB().then((db) => {
				db.collection(RECORDING_COLLECTION).countDocuments({}, (err, res) => {
					if (err) {
						reject(err);
					} else {
						resolve(res);
	...
	getTrackerHistoryOfRecording(recordingId) {
		return new Promise((resolve, reject) => {
			this.getDB().then((db) => {
				db.collection(TRACKER_COLLECTION)
					.find({ recordingId: ObjectID(recordingId) })
					.toArray(function (err, docs) {
						if (err) {
							reject(err);
						} else {
